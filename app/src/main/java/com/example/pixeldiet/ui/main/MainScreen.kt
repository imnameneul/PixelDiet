        ...
        Row(
            ...
            appsWithUsage.forEach { app ->
                ...
                if (app.icon != null) {
                    ...
                    AsyncImage(
                        model = app.icon,
                        contentDescription = app.appName.displayName,
                        modifier = Modifier.size(size)
                    )
                } else {
                    // 아이콘 없으면 기존 색 박스 사용 (fallback)
                    Box(
                        modifier = Modifier
                            .size(size)
                            .background(app.appName.composeColor)
                    )
                }
            }
        ...
        Row(modifier = Modifier.padding(16.dp), verticalAlignment = Alignment.CenterVertically) {
            ...
            */
            ...
            Column(
                ...
                // ⭐ 여기 새로 추가 ⭐
                Text(
                    text = formatTime(app.currentUsage),
                    fontSize = 16.sp,
                    color = Color.Black,
                    modifier = Modifier.padding(top = 4.dp)
                )
